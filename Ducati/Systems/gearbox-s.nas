...
#		Ducati Panigale 1299 S for Flightgear December 2016
	...
	if(running.getValue() == 1){
		...
		if (gear.getValue() > 0 and clutch.getValue() == 0) {
			...
			if(bwspeed < 3 and gspeed < 30){
				...
				#newrpm = (newrpm < lastrpm) ? (lastrpm - newrpm)/3 + newrpm: newrpm;
		...
		if(rpm.getValue() > maxrpm-2000 or speedlimstate.getBoolValue() == 1){
