...
setlistener("/surface-positions/left-aileron-pos-norm", func{
	...
	if (omm){
		if(cvnr == 0){
			setprop("/sim/current-view/x-offset-m", math.sin(position*0.15)*(0.4+driverpos/5));
			setprop("/sim/current-view/y-offset-m", math.cos(position*0.15)*(0.4+driverpos/5));
		...
		if(cvnr == 0){
			...
			interpolate("/sim/current-view/x-offset-m", math.sin(position*0.15)*(0.4+driverpos/5),0.1);
			interpolate("/sim/current-view/y-offset-m", math.cos(position*0.15)*(0.9 - godown/1500 + lookup*lookup*lookup/30 + driverpos/4),0.1);
			setprop("/sim/current-view/z-offset-m",driverview);	
		}    
	}
});