...
#		Motorcycle-Racing for Flightgear September 2016
#		This file is licenced under the terms of the GNU General Public Licence V2 or later
###########################################################################################
var flaginfo = props.globals.initNode("/controls/flag-info",0,"INT");
var racelap = props.globals.getNode("/Motorcycle/race-lap");
var thissector = props.globals.getNode("/Motorcycle/this-sector");
# only for mp result view
var sectortime = props.globals.getNode("/Motorcycle/this-sector-time");
var laptime = props.globals.getNode("/Motorcycle/this-lap-time");
var racetime = props.globals.getNode("/Motorcycle/this-race-time");
...
var total_result = func{
	var maxlaps = getprop("/Motorcycle/"~pa~"/race-laps") or 0;
	var tr = 0;
    for(v = 0; v <= maxlaps; v +=1) {
		var actl = getprop("/Motorcycle/"~pa~"/lap["~v~"]/actual-time") or 0;
		tr += actl;
	}
...
var show_lap_and_sector_time = func{
	var show_it = getprop("/Motorcycle/show-race-times") or 0;
	var show_l_or_f = getprop("/Motorcycle/switch-last-fastest") or 0;
	...
	foreach(var s; sectors) { 
		...
		var lsts = getprop("/Motorcycle/"~pa~"/sector["~ln~"]/start-time") or 0;
		var sts  = getprop("/Motorcycle/"~pa~"/sector["~n~"]/start-time") or 0;
		var lt  = getprop("/Motorcycle/"~pa~"/sector["~n~"]/last-time") or 0;
		var ft  = getprop("/Motorcycle/"~pa~"/sector["~n~"]/fastest-time") or 0;
		...
		if(thsec == n and racelap.getValue() > 0){
			...
			if(ct > 0){
				...
				setprop("Motorcycle/this-sector-time-sec", result[0]);
				setprop("Motorcycle/this-sector-time-min", result[1]);
			}
			if(ft > 0){
				var result = calc_time(ft);
				setprop("Motorcycle/fastest-sector-time-sec", result[0]);
				setprop("Motorcycle/fastest-sector-time-min", result[1]);
			}
			# only for mp
	...
	var fl = getprop("/Motorcycle/"~pa~"/fastest-lap") or 0;
	var ll = getprop("/Motorcycle/last-lap-time") or 0;
	...
	if(fl > 0){
		...
		setprop("Motorcycle/fastest-lap-time-sec", result[0]);
		setprop("Motorcycle/fastest-lap-time-min", result[1]);
		setprop("Motorcycle/fastest-lap-time-hour", result[2]);
	}
	if(lapt > 0){
		var result = calc_time(lapt);
		setprop("Motorcycle/this-lap-time-sec", result[0]);
		setprop("Motorcycle/this-lap-time-min", result[1]);
		setprop("Motorcycle/this-lap-time-hour", result[2]);
	}
	# only for mp
	...
	if(analyser.getBoolValue()&racelap.getValue()>0){
		...
		analyselapst += 1;
			...
			setprop("/instrumentation/Motorcycle/blackbox/lap["~racelap.getValue()~"]/lapstep["~analyselapst~"]/kmhormph", getprop("instrumentation/Motorcycle/speed-indicator/selection"));
	}
}
...
var find_marker = func{
	...
	if(getprop("/Motorcycle/"~pa~"/sector["~p~"]/fastest-time") == 0){
	...
	if(wp_distance < 260){
		var courserl = getprop("/Motorcycle/"~pa~"/race-laps") or 0;
	...
	if(wp_distance < 50){
		setprop("/Motorcycle/"~pa~"/sector["~thissector.getValue()~"]/start-time", getprop("/sim/time/elapsed-sec"));
		#print ("Sie ueberfuhren den "~thissector.getValue()~". Wegpunkt "~sectors[thissector.getValue()][3]);
		setprop("/Motorcycle/sector",sectors[thissector.getValue()][3]);

		var fastesttime = getprop("/Motorcycle/"~pa~"/sector["~ln~"]/fastest-time") or 0;
		var lastsectorstarttime = getprop("/Motorcycle/"~pa~"/sector["~ln~"]/start-time") or 0;
		var lastsectorendtime = getprop("/Motorcycle/"~pa~"/sector["~thissector.getValue()~"]/start-time") or 0;
		...
		setprop("/Motorcycle/"~pa~"/sector["~ln~"]/last-time", lasttime);
		...
		if(lasttime > 0 and lasttime < fastesttime or fastesttime == 0) setprop("/Motorcycle/"~pa~"/sector["~ln~"]/fastest-time", lasttime);
		...
		if(thissector.getValue() == 0){
		  ...
		  foreach(var ls; sectors) { 
		  	totallapresult += getprop("/Motorcycle/"~pa~"/sector["~n~"]/last-time") or 0;
			n+=1;
		  }
		  setprop("/Motorcycle/last-lap-time", totallapresult);
		  var fastestlap = getprop("/Motorcycle/"~pa~"/fastest-lap") or 0;
		  
		  # if actual time is less than fastest laptime
		  ldiff = (racelap.getValue() > 0) ? fastestlap - totallapresult : 0;
		  ######
		  
		  if(totallapresult > 0 and totallapresult < fastestlap or fastestlap == 0) setprop("/Motorcycle/"~pa~"/fastest-lap", totallapresult);
		  setprop("/Motorcycle/"~pa~"/lap["~racelap.getValue()~"]/actual-time", totallapresult);
		...
		setprop("/Motorcycle/this-lap-time-diff-s", resultldiff[0]);
		setprop("/Motorcycle/this-lap-time-diff-m", resultldiff[1]);
...
var clear_race_datas = func{
	...
	if(pa != nil and inrange != nil and inrange == 1){
		...
		laptime.setValue(0);
		racetime.setValue(0);
		var act = props.globals.getNode("/Motorcycle/"~pa);
		
		setprop("/Motorcycle/last-lap-time",0);
